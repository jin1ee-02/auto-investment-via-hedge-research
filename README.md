...
SEC 13F를 비교해 펀드의 공통 보유와 분기 변화를 탐색하고, TradingAgents로 종목을 추가 연구한 뒤 분기별 **모의 목표 배분안**을 저장하는 프로젝트입니다. 토스 공식 REST 연결 및 분기 1회 주문 실행 코드를 추가했습니다. 실거래는 기본 비활성이며, 최신 동작·제한·설정은 [분기 자동매매 안내](QUARTERLY_TRADING.md)를 참고하세요.
...
- `pipeline/toss.py`는 토스 인증·조회·주문 REST 어댑터이고 `pipeline/execution.py`가 분기별 중복과 수량 한도를 통제합니다. 기존 TossBrokerDisabled는 호환용으로 남아 있습니다. 실계좌 연결 테스트는 아직 하지 않았습니다.
...
- [토스증권 공식 API 문서](https://developers.tossinvest.com/docs) — 공식 명세 기반 REST 어댑터 구현, 실계좌 미검증
...
진행률은 공시 준비, 시장, 감성, 뉴스, 재무, 강세·약세 검토, 매매안, 리스크, 공통 변화 종합, 검증·저장의 10단계 완료 비율입니다. TradingAgents의 LangGraph 노드 완료 콜백에서 보고서 생성 여부를 확인합니다. 도구 호출을 위한 중간 반복은 완료로 계산하지 않습니다. 토큰 비율이나 남은 시간 예측이 아니며, 동일 단계에서 오래 걸릴 수 있습니다. 오류 시 마지막 진행 단계를 보존합니다.
...
TradingAgents는 시장·뉴스·재무 분석 후 강세/약세 연구자와 연구 책임자, Trader, 공격적/보수적/중립 리스크 분석자와 포트폴리오 책임자를 순차 실행합니다. 토론 설정은 각 1라운드입니다. 그 뒤 앱의 별도 심층 모델 호출이 13F 공통 매매 변화와 분기 보유 관점을 결합하고 구조화된 판단을 검증합니다. 감성 분석도 포함합니다. 원래 TradingAgents 최종 신호는 별도로 저장하고 주문 조건에 사용합니다. Trader/포트폴리오 책임자도 의견만 작성하며 주문하지 않습니다.
