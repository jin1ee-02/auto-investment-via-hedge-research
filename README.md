...
- 공시 점수에 따른 시뮬레이션, 종목 비중 상한/현금 하한, JSON 내보내기.
...
**미연결:** 사용자의 LLM API 키 및 모델. 실제 LLM 네트워크 실행은 검증하지 않았고 테스트 대역으로 오케스트레이션을 검증했습니다. 화면의 공시 점수는 AI 결과가 아닙니다. Python AI 의존성은 별도 가상환경에 설치해야 합니다.
...
5. 후보들의 연구가 완료되면 포트폴리오 설계에서 ‘AI 결과로 최종 배분안 생성’을 누릅니다. 현재 UI는 상위 8개 연구 후보를 표시합니다. 최대 종목 수를 8보다 크게 설정한 자동 연구는 아래 CLI로 실행하세요.
...
- 공시 점수: 보유 펀드 비율 60점 + 순확대 펀드 비율 25점 + 통합 비중 15점.
