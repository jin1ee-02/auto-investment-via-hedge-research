...
import {aggregate,allocate} from '../lib/domain.ts';
...
if(input.action==='candidates')process.stdout.write(JSON.stringify({period:dataset.period,previousPeriod:dataset.previousPeriod,generatedAt:dataset.generatedAt,rows:rows.filter(r=>r.ticker&&r.holders>0).sort((a,b)=>b.score-a.score)}));
else if(input.action==='allocate'){
 const scored=rows.filter(r=>Object.hasOwn(input.scores,r.ticker)).map(r=>{const score=input.scores[r.ticker];if(typeof score!=='number'||!Number.isFinite(score)||score<0||score>100)throw Error('Invalid AI score');return {...r,score}});
