...
if(input.action==='candidates')process.stdout.write(JSON.stringify({period:dataset.period,previousPeriod:dataset.previousPeriod,generatedAt:dataset.generatedAt,rows:researchCandidates(rows,input.count,input.candidateFilter)}));
else if(input.action==='allocate'){
 const scored=researchCandidates(rows,undefined,input.candidateFilter).filter(r=>r.buyers>=2&&r.buyers>r.sellers&&Object.hasOwn(input.scores,r.ticker)).map(r=>{const score=input.scores[r.ticker];if(typeof score!=='number'||!Number.isFinite(score)||score<0||score>100)throw Error('Invalid AI score');return {...r,score}});
